...
client.on("messageCreate", async (message) => {
  ...
  if (cmd.length === 0) {
    if (matchedPrefix.includes(client.user.id)) {
      message.reply(
        `Hey, **Bot** is now using Slash Commands!
\<@${message.author.id}>To see all Commands type: \`/help\` \n \n   Don't see the slash commands? Please try reinviting the bot or wait for about 1-2 hours.\n  \n If you continue to have problems, consider asking for help on our **discord server**\``
      );
    }
  }
